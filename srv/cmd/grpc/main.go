...
func (s *GeocoderServer) InsertorReplaceAddressData(stream pb.Geocoder_InsertorReplaceAddressDataServer) (err error) {

	// In lieu of a proper middleware log all incoming requests here
	var (
		startTime             = time.Now() // call on entry as proxy for use w. cobbled-together request logger
		numQueuedTransactions int          // current number of un-submitted addresses in redis exec pipeline
		jobSuccess            bool         // success flag for insertion request; returned as part of pb.IOResponse
		totalObjectsWritten   int          // total addresses "committed" to redis; returned as part of pb.IOResponse
		respCode              = codes.OK   // status code; returned as part of pb.IOResponse
	)
...
func (s *GeocoderServer) Geocode(ctx context.Context, req *pb.GeocodeRequest) (*pb.GeocodeResponse, error) {
	...
	var (
		startTime      = time.Now()        // call on entry as proxy for use w. cobbled-together request logger
		respCode       = codes.OK          // status code; returned as part of pb.IOResponse
		err            error               // error; returned as part of pb.IOResponse
		addressResults []*pb.ScoredAddress // list of result addresses; returned as part of pb.IOResponse
	)
	...
	default:
		return nil, status.Errorf(codes.Unimplemented, "unknown code path")
	}
}
...
func (s *GeocoderServer) indexesReady() error {
	...
	// TODO: check does NOHL or NOOFFSETS improve performance in any meaningful way (?)
	_, err := s.client.Do(
	...
	log.Info("created new FT index on server init")
	return nil
}

func init() {
	log.SetFormatter(&log.JSONFormatter{TimestampFormat: "2006-01-02 15:04:05.99999"})
...
func main() {
	...
	// listen on address an port from flags
	lis, err := net.Listen("tcp", fmt.Sprintf("%s:%d", *serverListenAddr, *serverPort))
	if err != nil {
		log.WithFields(log.Fields{
			"bindAddress": *serverListenAddr,
			"bindPort":    *serverPort,
			"err":         err,
		}).Panic("failed to listen on address")
	}

	// NOTE: Register reflection service on gRPC server for grpc_cli testing
